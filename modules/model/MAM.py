...
from modules.model.mamba import MambaModel

NO_INIT_POSE = False


class SSM_JP_estimator(nn.Module):
    name = "RNN_JP"
    imu_m = ["vacc", "vrot"]
    ...
    @staticmethod
    def get_config(args):
        config_str = f"{args.network}-input{SSM_JP_estimator.imu_m},-output{SSM_JP_estimator.model_output}"
        return config_str
    ...
    def __init__(self, args):
        ...
        self.rnn_leaf_joint_mapper = MambaModel(d_input=self.input_size,
                                                d_output=joint_set.n_leaf * 3,
                                                d_model=args.n_hidden,
                                                n_layers=2,
                                                dropout=0.2,
                                                prenorm=False,
                                                bidirectional=False
    ...
    def forward(self, x):
        ...
        leaf_joint = self.rnn_leaf_joint_mapper(list(x_imu))

        return leaf_joint


class Mamba(BaseModel):
    name = "MAM Inertial Poser"
    ...
    @staticmethod
    def add_args(parser):
        SSM_JP_estimator.add_args(parser)
        Graph_JP_estimator.add_args(parser)
        group = parser.add_argument_group("SSM Config")
        group.add_argument('--add_gaussian', action='store_true',
                           help='Whether to add gaussian to leaf joints estimation')
        group.add_argument('--acc_min_fuse', type=float, default=1.0,
                           help='The lowerb threshold to fuse joint position of uwb and imu measurement')
        group.add_argument('--acc_max_fuse', type=float, default=3.0,
                           help='The upperb threshold to fuse joint position of uwb and imu measurement')
    ...
    @staticmethod
    def get_config(args):
        config_str = f"{args.network}-input{SSM.imu_m},-output{SSM.model_output}"
        config_str += f"-input_sensor{SSM.imu_m}-hidden_dim{args.hidden_dim}"
        return config_str
    ...
    def __init__(self, args) -> None:
        super().__init__(args)
        self.rnn_jp_mapper = SSM_JP_estimator(args)
        ...
        self.rnn4 = MambaModel(d_input=self.input_size + joint_set.n_leaf * 3,
                            d_output=24 * 3,
                            d_model=args.n_hidden,
                            n_layers=3,
                            dropout=0.2,
        ...
        self.rnn3 = MambaModel(d_input=self.input_size + joint_set.n_leaf * 3,
                            d_output=joint_set.n_reduced * 6,
                            d_model=args.n_hidden,
                            n_layers=3,
                            dropout=0.2,
                            prenorm=False,
        ...
        self.rnn5 = MambaModel(d_input=self.input_size + joint_set.n_leaf * 3,
                            d_output=2,
                            d_model=32,
                            n_layers=3,
                            dropout=0.2,
    ...
    def fuse_joint_position_estimation(self, jp1, jp2, beta):
        ...
        beta_pooled = torch.repeat_interleave(pooled_acc_mag, repeats=self.max_pool.kernel_size, dim=-1)[:, :,
                      :f].permute(0, 2, 1)
        assert beta_pooled.size() == beta.size()

        deno = self.acc_ub - self.acc_lb
        weight_jp1 = torch.min(torch.max((beta_pooled - self.acc_lb) / deno, torch.zeros_like(beta_pooled)),
                               torch.ones_like(beta_pooled)).unsqueeze(-1)
        weight_jp2 = torch.min(torch.max((beta_pooled - self.acc_ub) / -deno, torch.zeros_like(beta_pooled)),
                               torch.ones_like(beta_pooled)).unsqueeze(-1)
    ...
    def forward(self, x):
        ...
        x, lj_init, jvel_init = list(zip(*x))
        imu_acc_leaf_norm = torch.stack(
            [torch.norm(_[:, :(self.imu_num - 1) * 3].view(-1, self.imu_num - 1, 3), dim=-1) for _ in list(x)])

        leaf_joint = self.fuse_joint_position_estimation(jp1=torch.stack(leaf_joint_rnn),
                                                         jp2=torch.stack(leaf_joint_gnn), beta=imu_acc_leaf_norm)
        leaf_joint_input = [leaf_joint[i] + torch.normal(0, 0.04, size=leaf_joint[0].size()).to(leaf_joint[i].device)
                            for i in range(len(leaf_joint))] if self.training and self.add_guassian else leaf_joint

        global_6d_pose = self.rnn3([torch.cat(_, dim=-1) for _ in zip(leaf_joint_input, x)])
        joint_velocity = self.rnn4(list([torch.cat(_, dim=-1) for _ in zip(leaf_joint_input, x)]))
    ...
    def predict(self, glb_acc, glb_rot, init_pose, **kwargs):
        ...
        x = [_.to('cuda') for _ in x]
        leaf_joint_rnn, leaf_joint_gnn, global_6d_pose, joint_velocity, contact = [_[0] for _ in self.forward([x])]
        leaf_joint_rnn, leaf_joint_gnn, global_6d_pose, joint_velocity, contact =leaf_joint_rnn.to('cpu'), leaf_joint_gnn.to('cpu'), global_6d_pose.to('cpu'), joint_velocity.to('cpu'), contact.to('cpu')
