...
def evaluate_model(net, data_dir, sequence_ids=None, flush_cache=False, pose_evaluator=PoseEvaluator(),
			 evaluate_pose=True, evaluate_tran=False, evaluate_zmp=False, plt_tran=False, device='cpu', **kwargs):
	r"""
	...
	_, _, pose_t_all, tran_t_all, *res = torch.load(os.path.join(data_dir, 'test.pt'), weights_only=True).values() #[45,Size([4113, 24, 3])],[45,torch.Size([4113, 3])]
	...
	for i in tqdm.tqdm(sequence_ids):
		result = torch.load(os.path.join(result_dir, '%d.pt' % i), weights_only=True)
...
def main():
	...
	if args.network in MODELS:
		...
		eval_res = evaluate_model(net,data_dir=args.data_dir,sequence_ids=sequence_ids,flush_cache=args.flush_cache,evaluate_tran=args.eval_trans,pose_evaluator=PoseEvaluator(),
                          			normalize_uwb=args.normalize_uwb,flatten_uwb=args.flatten_uwb, device =args.device)
		y_axis_up = True
